...
protected[trembita]
class ParMapList[+A, B](f: A => B, source: LazyList[A])
...
protected[trembita]
class ParFlatMapList[+A, B](f: A => Iterable[B], source: LazyList[A])
...
protected[trembita]
class ParCollectList[+A, B](pf: PartialFunction[A, B], source: LazyList[A])
...
protected[trembita]
class ParSource[+A](iterable: => Iterable[A])(implicit val ec: ExecutionContext) extends BaseLazyList[A] with ParLazyList[A] {
...
protected[trembita]
class ParSortedSource[+A: Ordering : ClassTag](source: LazyList[A])
