...
protected[trembita] class MapList[+A, B](f: A => B, source: LazyList[A]) extends BaseLazyList[B] {
...
protected[trembita] class FlatMapList[+A, B](f: A => Iterable[B], source: LazyList[A]) extends BaseLazyList[B] {
...
protected[trembita] class CollectList[+A, B](pf: PartialFunction[A, B], source: LazyList[A]) extends BaseLazyList[B] {
...
protected[trembita] trait SeqSource[+A] extends BaseLazyList[A] {
...
protected[trembita] class StrictSource[+A](iterable: => Iterable[A]) extends SeqSource[A] {
...
protected[trembita] class CachedSource[+A](iterable: Iterable[A]) extends SeqSource[A] {
  override def force: Iterable[A] = iterable
  override def par(implicit ec: ExecutionContext): ParLazyList[A] = new ParSource[A](iterable)
  override def take(n: Int): Iterable[A] = iterable.take(n).toVector
  override def drop(n: Int): Iterable[A] = iterable.drop(n).toVector
...
protected[trembita] class IteratorSource[+A](xiterator: Iterator[A]) extends SeqSource[A] {
...
protected[trembita] class SortedSource[+A: Ordering : ClassTag](source: LazyList[A]) extends SeqSource[A] {
