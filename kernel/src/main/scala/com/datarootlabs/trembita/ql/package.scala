...
package object ql
  ...
  implicit class TrembitaQL[A](val self: Seq[A]) extends AnyVal {
    ...
    (implicit trembitaql: trembitaql[A, G, T, R, Comb]): ArbitraryGroupResult[A, G, AggFunc.Result[T, R, Comb]] =
  ...
  implicit class TrembitaQLForPipeline[A](val self: DataPipeline[A]) extends AnyVal {
    ...
    : DataPipeline[ArbitraryGroupResult[A, G, AggFunc.Result[T, R, Comb]]] = DataPipeline.from({
  ...
  implicit def having2Query[A, G <: GroupingCriteria, T <: AggDecl, R <: AggRes, Comb]
  (h: Having[A, G, T, R, Comb]): Query[A, G, T, R, Comb] =
