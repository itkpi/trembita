...
trait orderingInstances {
  ...
  implicit val durationOrdering     : Ordering[FiniteDuration] = Ordering.fromLessThan(_ <= _)

  implicit def taggedOrdering[A: Ordering, U]: Ordering[A :@ U] = Ordering.by(_.value)
  ...
  (implicit GH: Ordering[GH], GT: Ordering[GT]): Ordering[GH &:: GT] = new Ordering[GH &:: GT] {
    def compare(x: GH &:: GT, y: GH &:: GT): Int = GH.compare(x.head, y.head) match {
  ...
  (implicit GH: Ordering[RH], GT: Ordering[RT]): Ordering[RH *:: RT] = new Ordering[RH *:: RT] {
    def compare(x: RH *:: RT, y: RH *:: RT): Int = GH.compare(x.head, y.head) match {
      case 0     => GT.compare(x.tail, y.tail)
      case other => other
    }
  }
