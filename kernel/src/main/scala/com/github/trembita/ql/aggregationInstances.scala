...
trait aggregationInstances extends Serializable {
  ...
  implicit def taggedAggArr[A, U]: AggFunc[TaggedAgg[A, U, AggFunc.Type.Arr], Vector[A] :@ U, Vector[A]] =
    new AggFunc[TaggedAgg[A, U, AggFunc.Type.Arr], Vector[A] :@ U, Vector[A]] {
      ...
      def add(comb: Vector[A], value: TaggedAgg[A, U, AggFunc.Type.Arr]): Vector[A] =
        comb :+ value.tagged.value
  ...
  /**
    ...
    **/
  implicit def taggedAggStandardDeviation[A, U](implicit FA: Field[A], NRoot: NRoot[A]): AggFunc[TaggedAgg[
    A,
    U,
    AggFunc.Type.STDEV
  ...
  /**
    ...
    **/
  implicit def aggConsAggFunc[A, U, AggF <: AggFunc.Type, AggHComb, HOut <: :@[_, _], T <: AggDecl, AggTComb, TOut <: AggRes](
