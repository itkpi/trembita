...
object Execution {
  ...
  sealed trait Sequential extends Execution {
    ...
    def collect[A, B](repr: Vector[A])(pf: PartialFunction[A, B]): Vector[B] =
      repr.collect(pf)
    ...
    def groupBy[A, K](vs: Vector[A])(f: A => K): Map[K, Vector[A]] =
      vs.groupBy(f)
  ...
  sealed trait Parallel extends Execution {
    ...
    def collect[A, B](repr: ParVector[A])(
    ...
    def zip[A, B](xs: ParVector[A], ys: ParVector[B]): ParVector[(A, B)] =
      xs.zip(ys)

    val Monad: Monad[ParVector] = new Monad[ParVector] {
      def pure[A](a: A): ParVector[A] = ParVector(a)

      def flatMap[A, B](fa: ParVector[A])(f: A => ParVector[B]): ParVector[B] =
        fa.flatMap(f)
      ...
      override def map[A, B](fa: ParVector[A])(f: A => B): ParVector[B] =
        fa.map(f)
    }

    def groupBy[A, K](vs: ParVector[A])(f: A => K): Map[K, ParVector[A]] =
      vs.groupBy(f).seq

    val Traverse: Traverse[ParVector] = new Traverse[ParVector] {
      def traverse[G[_], A, B](
        fa: ParVector[A]
      )(f: A => G[B])(implicit G: Applicative[G]): G[ParVector[B]] = {
        foldRight[A, G[ParVector[B]]](fa, Always(G.pure(ParVector.empty))) {
          (a, lgvb) =>
