...
trait ToCaseClass[A, K <: GroupingCriteria, T] extends DepFn1[QueryResult[A, K, T]] with Serializable
...
object ToCaseClass {
  ...
  implicit def derive[A, K <: GroupingCriteria, KR <: HList, T, TR <: HList, P <: HList, L <: HList, R](
      implicit kToHList: ToHList.Aux[K, KR],
