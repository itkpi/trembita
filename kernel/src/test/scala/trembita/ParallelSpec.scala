...
class ParallelSpec extends FlatSpec {
  ...
  "DataPipeline operations" should "not be executed until 'eval'" in {
    val pipeline = Input.parallel[IO, Throwable, Seq].create(Seq(1, 2, 3))
  ...
  "DataPipeline.collect(toInt)" should "be DataPipeline[Int]" in {
    val pipeline = Input.parallel[IO, Throwable, Seq].create(Seq("1", "2", "3", "abc"))
  ...
  "DataPipeline.flatMap(getWords)" should "be a pipeline of words" in {
    val pipeline            = Input.parallel[IO, Throwable, Seq].create(Seq("Hello world", "hello you to"))
  ...
  "DataPipeline.sorted" should "be sorted" in {
    val pipeline            = Input.parallel[IO, Throwable, Seq].create(Seq(5, 4, 3, 1))
  ...
  "DataPipeline.sortBy(_.length)" should "be sorted by length" in {
    val pipeline            = Input.parallel[IO, Throwable, Seq].create(Vector("a", "abcd", "bcd"))
  ...
  "Output.reduce(_+_)" should "produce pipeline sum" in {
    val pipeline                    = Input.parallel[IO, Throwable, Seq].create(Vector(1, 2, 3))
  ...
  "Output.reduce" should "throw NoSuchElementException on empty pipeline" in {
    val pipeline = Input.parallel[IO, Throwable, Seq].empty[Int]
  ...
  "Output.combineAll" should "work correctly" in {
    val pipeline                    = Input.parallel[IO, Throwable, Seq].create(Seq(1, 2, 3, 4, 5))
  ...
  "DataPipeline.groupBy" should "group elements" in {
    val pipeline = Input.parallel[IO, Throwable, Seq].create(Seq(1, 2, 3, 4))
  ...
  "DataPipeline.distinct" should "work" in {
    val pipeline              = Input.parallel[IO, Throwable, Seq].create(Seq(1, 2, 3, 1, 3, 2, 1))
  ...
  "PairPipeline transformations" should "work correctly" in {
    val pipeline = Input.parallel[IO, Throwable, Seq].create(Seq("a" -> 1, "b" -> 2, "c" -> 3))
  ...
  "DataPipeline.zip" should "work correctly for pipelines" in {
    val p1                            = Input.parallel[IO, Throwable, Seq].create(Seq(1, 2, 3, 4))
    val p2                            = Input.parallel[IO, Throwable, Seq].create(Seq("a", "b", "c"))
  ...
  "DataPipeline.++" should "work correctly for pipelines" in {
    val p1                  = Input.parallel[IO, Throwable, Seq].create(Seq(1, 2, 3, 4))
    val p2                  = Input.parallel[IO, Throwable, Seq].create(Seq(5, 6, 7))
  ...
  "DataPipeline transformations wrapped in Kleisli" should "be evaluated correctly" in {
    val pipeline = Input.parallelF[IO, Throwable, Seq].create(IO(Seq(1, 2, 3, 4, 5)))
    val pipe = biPipe[IO, Throwable, Int, String, Parallel](
      _.mapM(i => IO { i + 1 })
        .filter(_ % 2 == 0)
        .map(_.toString)
    )
  ...
  "DataPipeline.mapError" should "work correctly" in {
    val pipeline = Input.parallelF[IO, Throwable, Seq].create(IO(Seq(0, 1, 2, 3, 4, 5)))
    val evaled = pipeline
      .map(10 / _)
      .mapError {
    ...
    val evaled4 = pipeline
      .flatMap(x => Input.parallelF[IO, Throwable, Seq].create(IO { Seq(10 / x, 20 / x) }))
      .map(_ + 1)
      .mapConcat(_ to 5)
      .mapError {
  ...
  it should "allow to handle error" in {
    ...
    val pipeline = Input.parallelF[IO, Throwable, Seq].create(IO(seq))
    val evaled = pipeline
      .map(10 / _)
      .mapError {
