...
class JstreamsParSpec extends FlatSpec {
  ...
  "DataPipeline.collect(toInt)" should "be DataPipeline[Int]" in {
    val pipeline = Input.jstream[StreamType.Parallel].create(Seq("1", "2", "3", "abc"))
  ...
  "DataPipeline[String, Try, ...].mapM(...toInt)" should "be DataPipeline[Int]" in {
    val pipeline = Input.jstreamF[Try, StreamType.Parallel].create(Try(Vector("1", "2", "3", "abc")))
    val res = pipeline
      .mapM { str =>
        Try(str.toInt)
      }
  ...
  "DataPipeline.flatMap(getWords)" should "be a pipeline of words" in {
    val pipeline            = Input.jstream[StreamType.Parallel].create(Seq("Hello world", "hello you to"))
  ...
  "DataPipeline.sorted" should "be sorted" in {
    val pipeline            = Input.jstream[StreamType.Parallel].create(Seq(5, 4, 3, 1))
  ...
  "DataPipeline.sortBy(_.length)" should "be sorted by length" in {
    val pipeline            = Input.jstream[StreamType.Parallel].create(Vector("a", "abcd", "bcd"))
  ...
  "Output.reduce(_+_)" should "produce pipeline sum" in {
    val pipeline = Input.jstream[StreamType.Parallel].create(Vector(1, 2, 3))
  ...
  "Output.reduce" should "throw NoSuchElementException on empty pipeline" in {
    val pipeline = Input.jstream[StreamType.Parallel].empty[Int]
  ...
  "PairPipeline transformations" should "work correctly" in {
    val pipeline = Input.jstream[StreamType.Parallel].create(Seq("a" -> 1, "b" -> 2, "c" -> 3))
  ...
  "DataPipeline.zip" should "work correctly for pipelines" in {
    val p1                            = Input.jstream[StreamType.Parallel].create(Seq(1, 2, 3, 4))
    val p2                            = Input.jstream[StreamType.Parallel].create(Seq("a", "b", "c"))
  ...
  "DataPipeline.++" should "work correctly for pipelines" in {
    val p1                  = Input.jstream[StreamType.Parallel].create(Seq(1, 2, 3, 4))
    val p2                  = Input.jstream[StreamType.Parallel].create(Seq(5, 6, 7))
  ...
  "DataPipeline.drop" should "work correctly" in {
    val pipeline            = Input.jstream[StreamType.Parallel].create(Seq(1, 2, 3, 4))
    val result: Vector[Int] = pipeline.drop(2).into(Output.vector).run
    assert(result == Vector(3, 4))
  }

  "DataPipeline.distinct" should "work" in {
    val pipeline              = Input.jstream[StreamType.Parallel].create(Seq(1, 2, 3, 1, 3, 2, 1))
  ...
  "DataPipeline transformations wrapped in Kleisli" should "be evaluated correctly" in {
    val pipeline = Input.jstreamF[IO, StreamType.Parallel].create(IO(Seq(1, 2, 3, 4, 5)))
    val pipe = biPipeT[IO, Int, String, JavaStreams[StreamType.Parallel]](
      _.mapM(i => IO { i + 1 })
        .filter(_ % 2 == 0)
        .map(_.toString)
    )
