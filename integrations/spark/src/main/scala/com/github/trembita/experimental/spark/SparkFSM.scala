...
import com.github.trembita.fsm.FSM.Func
...
trait SparkFSM[F[_]] {
  def byKey[A: ClassTag: Encoder,
            K: ClassTag: Encoder,
            N,
            D,
            B: TypeTag: Encoder](source: RDD[A])(
    getKey: A => K,
...
class SparkIDFsm(implicit val spark: SparkSession) extends SparkFSM[Id] {
  ...
  def byKey[A: ClassTag: Encoder,
    ...
    val mappedDS: Dataset[Vector[B]] = ds
      .groupByKey(getKey)
      .mapGroupsWithState[FSM.State[N, D, Id], Vector[B]] {
        (k: K, as: Iterator[A], groupState: GroupState[FSM.State[N, D, Id]]) =>
